...
namespace CPBase.Geo.Numerics
{
    public struct Size3D : IFormattable
    {
        ...
        //
        // Summary:
        //     Converts this System.Windows.Media.Media3D.Size3D structure into a System.Windows.Media.Media3D.Vector3
        //     structure.
        //
        // Parameters:
        //   size:
        ...
        {
            return new Vector3(size._x, size._y, size._z);
        }

        private static Size3D CreateEmptySize3D()
        {
            Size3D result = default;
            result._x = float.NegativeInfinity;
            result._y = float.NegativeInfinity;
            result._z = float.NegativeInfinity;
            return result;
        }
        ...
        /*public static Size3D Parse(string source)
        {
            ...
            Size3D result = ((!(text == "Empty")) ? new Size3D(Convert.Tofloat(text, invariantEnglishUS), Convert.Tofloat(tokenizerHelper.NextTokenRequired(), invariantEnglishUS), Convert.Tofloat(tokenizerHelper.NextTokenRequired(), invariantEnglishUS)) : Empty);
